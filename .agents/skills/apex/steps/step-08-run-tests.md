...
**If `{auto_mode}` = true:**
→ Use the test command's own server lifecycle. Playwright starts CESIZen through
`playwright.config.ts`; do not launch a detached development server by default.
...
| Error | Solution |
|-------|----------|
| Cannot find module | Check imports and run `bun install` |
