...
Look for: `type-check`, `lint`, `test`, `build`
...
**MUST PASS.** If fails:
1. Run `bun run lint -- --fix` when an automatic fix is appropriate
...
Run a formatter only when `package.json` exposes one. CESIZen currently does
not define a formatting script.
