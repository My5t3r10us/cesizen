...
### 0. Verify Git Metadata

```bash
git rev-parse --is-inside-work-tree
```

If the command fails, record that Git metadata is unavailable, disable branch
and PR modes, and return to initialization. Do not treat the missing `.git`
directory as an implementation failure.
