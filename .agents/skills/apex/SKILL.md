---
name: apex
description: Systematic implementation using APEX methodology (Analyze-Plan-Execute-eXamine) with optional parallel subagents, self-validation, and adversarial review. Use when implementing features, fixing bugs, or making code changes that benefit from structured workflow.
...
/apex -a -m implement full auth system        # Parallel subagents
...
| `-m` | `-M` | `--teams` | Parallel subagent execution (enables -k) |
...
5. **Execute** → Implementation (standard or parallel subagents if -m)
